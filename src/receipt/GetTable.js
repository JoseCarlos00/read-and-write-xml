...
export class GetTable {
	constructor(Receipt) {
		this.ReceiptDetails = Receipt.Details?.[0]?.ReceiptDetail ?? [];
	}
	 ...
	 */
	get totalLines() {
		return this.ReceiptDetails.length;
	}
	...
	async getTbody() {
		...
		const sortedDetails = this.ReceiptDetails.sort((a, b) => {
...
>
	 ...
	 * @param {Object} receipt - Objeto de envío con detalles de `receipt`.
	 * @return {HTMLElement|null} La tabla generada o null si no hay datos de envío.
	 */
	static async getTableElement(receipt) {
		if (!receipt) {
			return null;
		}

		const getTable = new GetTable(receipt);

		const table = document.createElement("table");
		table.id = "receiptDetailsTable";
